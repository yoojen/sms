...
from models.models import (
    Department, Admin, TeacherDepartments, BaseModel, Teacher
)
...
def teacher_dept():
    ...
    new_obj = {}
    all_associations = []
    tchr_dept_associations = db_controller.get_all_object(TeacherDepartments)
    if tchr_dept_associations:
        for td in tchr_dept_associations:
            teacher = [
                td.teacher.to_dict() if td.teacher else None]
            department = [
...
def single_teacher_dept(id):
    ...
    new_obj = {}
    td = db_controller.get_by_id(TeacherDepartments, id)
    if td:
        teacher = [
            td.teacher.to_dict() if td.teacher else None]
        department = [
...
def create_teacher_association():
    ...
    teacher = db_controller.get_by_id(Teacher, int(data['teacher_id']))
    if not teacher:
    ...
    try:
        # check if it exists
        assoc = db_controller.search(TeacherDepartments, **data)
        if assoc:
            return jsonify(ERROR='Association alredy exists'), 409
        created = db_controller.create_object(TeacherDepartments(**data))
...
def update_association_object(id):
    ...
    try:
        ...
        if assoc:
            return jsonify(ERROR='Association alredy exists'), 409
        updated = db_controller.update(TeacherDepartments, id, **data)
...
def remove_association(id):
    ...
    try:
        db_controller.delete(TeacherDepartments, id)
